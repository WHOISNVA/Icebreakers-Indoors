...
function navigationUrl(lat: number, lng: number): string {
  // This URL format opens Google Maps directly in navigation mode
  const destination = `${lat},${lng}`;
  
  if (Platform.OS === 'ios') {
    // iOS: Use the comgooglemaps-x-callback URL scheme for immediate navigation
    // The x-source and x-success parameters help with app switching
    return `comgooglemaps-x-callback://?daddr=${destination}&directionsmode=driving&x-success=sourceapp://?resume=true&x-source=BarConnect`;
  } else {
    // Android: Use the google.navigation intent with immediate start
    return `google.navigation:q=${destination}&mode=d&navigate=yes`;
  }
}

// Alternative: Use maps.google.com with navigation intent
function webNavigationUrl(lat: number, lng: number): string {
  // This opens Google Maps web with navigation auto-starting
  // The 'nav=1' parameter tells Google Maps to start navigation immediately
  return `https://maps.google.com/maps?daddr=${lat},${lng}&amp;ll=&amp;z=15&amp;t=m&amp;hl=en-US&amp;gl=US&amp;mapclient=apiv3&amp;nav=1`;
}
...
export default function BartenderScreen() {
  ...
  const openNavigation = async (o: Order) => {
    ...
    if (Platform.OS === 'ios') {
      // On iOS, try Apple Maps first as it auto-starts navigation
      const appleMapsUrl = `maps://maps.apple.com/?daddr=${location.latitude},${location.longitude}&dirflg=d`;
      
      try {
        const canOpenAppleMaps = await Linking.canOpenURL(appleMapsUrl);
        if (canOpenAppleMaps) {
          // Apple Maps will auto-start navigation
          await Linking.openURL(appleMapsUrl);
          return;
        }
      } catch (error) {
        console.log('Apple Maps not available:', error);
      }
    }
