...
function navigationUrl(lat: number, lng: number): string {
  ...
  // Using the google.navigation scheme for direct navigation
  const destination = `${lat},${lng}`;
  
  // Try the navigation intent first (works on mobile)
  if (Platform.OS === 'ios') {
    // iOS: Use the comgooglemaps URL scheme for navigation
    return `comgooglemaps://?daddr=${destination}&directionsmode=driving`;
  } else {
    // Android: Use the google.navigation intent
...
function webNavigationUrl(lat: number, lng: number): string {
  // This opens in browser with navigation ready to start
