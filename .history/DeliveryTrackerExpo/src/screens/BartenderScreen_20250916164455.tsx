...
import { View, Text, StyleSheet, FlatList, TouchableOpacity, Linking, RefreshControl, Alert, Platform } from 'react-native';
...
function navigationUrl(lat: number, lng: number): string {
  // This URL format opens Google Maps with Live View/AR navigation
  const destination = `${lat},${lng}`;
  
  if (Platform.OS === 'ios') {
    // iOS: Use the comgooglemaps URL scheme with walking mode to trigger Live View
    // Walking mode is required for Live View to be available
    return `comgooglemaps://?daddr=${destination}&directionsmode=walking&views=ar`;
  } else {
    // Android: Use the google.navigation intent with walking mode
    // The 'mode=w' enables walking navigation which supports Live View
    return `google.navigation:q=${destination}&mode=w`;
  }
}

// Alternative web-based navigation URL as fallback
function webNavigationUrl(lat: number, lng: number): string {
  // This opens in browser with walking directions (Live View available on mobile web)
...
export default function BartenderScreen() {
  ...
  const openNavigation = async (o: Order) => {
    // Use current location if available, otherwise use origin
    const location = o.currentLocation || o.origin;
    console.log('Opening Live View navigation for order:', o.id);
    console.log('Using location:', location);
    
    // Note: Live View requires walking directions and may need user to tap the Live View button
    // Google Maps doesn't allow direct deep linking to Live View, but walking mode makes it available
    Alert.alert(
      'Opening Live View',
      'Google Maps will open in walking mode. Look for the "Live View" button (camera icon) at the bottom of the screen to start AR navigation.',
      [
        {
          text: 'Cancel',
          style: 'cancel'
        },
        {
          text: 'Continue',
          onPress: async () => {
            const navUrl = navigationUrl(location.latitude, location.longitude);
            const webUrl = webNavigationUrl(location.latitude, location.longitude);
            
            try {
              // Try native navigation URL first
              ...
              console.log('Navigation error, using web fallback:', error);
  ...
  const renderOrder = ({ item }: { item: Order }) => (
    <View style={styles.card} key={`${item.id}-${item.currentLocation?.timestamp || 'origin'}`}>
      ...
      <View style={styles.actions}>
        ...
        >
          <Text style={styles.btnText}>📷 Live View</Text>
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  btn: { backgroundColor: '#007AFF', paddingVertical: 10, paddingHorizontal: 12, borderRadius: 10, flex: 1 },
  btnText: { color: '#fff', fontWeight: '700', fontSize: 13, textAlign: 'center' },
  navigationBtn: { backgroundColor: '#34C759' },
