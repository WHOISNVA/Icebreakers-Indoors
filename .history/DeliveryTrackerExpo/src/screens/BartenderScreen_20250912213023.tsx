  ...
  const renderOrder = ({ item }: { item: Order }) => (
    <View style={styles.card}>
      ...
      {item.locationDescription && (
        <View style={styles.locationDescContainer}>
          <Text style={styles.locationDescLabel}>📍 Location:</Text>
          <Text style={styles.locationDesc}>{item.locationDescription}</Text>
        </View>
      )}
      
      <Text style={styles.meta}>
        GPS: {item.origin.latitude.toFixed(6)}, {item.origin.longitude.toFixed(6)}
      ...
      {item.currentLocation && (
        <>
          {item.currentLocationDescription && (
            <View style={styles.locationDescContainer}>
              <Text style={styles.locationDescLabel}>📍 Updated Location:</Text>
              <Text style={styles.locationDesc}>{item.currentLocationDescription}</Text>
            </View>
          )}
          <Text style={styles.meta}>
            Current GPS: {item.currentLocation.latitude.toFixed(6)}, {item.currentLocation.longitude.toFixed(6)}
      ...
      <View style={styles.actions}>
        <TouchableOpacity style={styles.btn} onPress={() => openMap(item)}>
...
const styles = StyleSheet.create({
  ...
  meta: { color: '#3C3C43', marginBottom: 2, fontSize: 12 },
