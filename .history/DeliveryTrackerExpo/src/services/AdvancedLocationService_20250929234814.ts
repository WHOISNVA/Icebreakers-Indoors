...
class KalmanFilter {
  ...
  update(reading: GNSSReading): { latitude: number; longitude: number } {
    ...
    const innovation = {
      latitude: reading.latitude - prediction.latitude,
      longitude: reading.longitude - prediction.longitude
    };
    ...
    this.lastEstimate = {
      ...reading,
      latitude: prediction.latitude + K * innovation.latitude,
      longitude: prediction.longitude + K * innovation.longitude
    };

    this.P = (1 - K) * this.P;
    
    return { latitude: this.lastEstimate.latitude, longitude: this.lastEstimate.longitude };
  }
  ...
  private predict(): { latitude: number; longitude: number } {
    if (!this.lastEstimate) {
      return { latitude: 0, longitude: 0 }; // Initial state
    }
    const timeDelta = (this.lastEstimate.timestamp - Date.now()) / 1000; // Assuming timestamp is in milliseconds
    return {
      latitude: this.lastEstimate.latitude + this.lastEstimate.speed * Math.cos(this.lastEstimate.heading * Math.PI / 180) * timeDelta,
      longitude: this.lastEstimate.longitude + this.lastEstimate.speed * Math.sin(this.lastEstimate.heading * Math.PI / 180) * timeDelta
    };
  }
}
