...
import { Point3D, BeaconAnchor, UWBAnchor } from './Venue3DMapService';
...
class EnhancedARVIOService {
  ...
  updateBeaconReading(reading: BeaconReading): void {
    ...
    Array.from(this.recentBeacons.entries()).forEach(([uuid, beacon]) => {
  ...
  updateUWBReading(reading: UWBReading): void {
    ...
    Array.from(this.recentUWB.entries()).forEach(([anchorId, uwb]) => {
