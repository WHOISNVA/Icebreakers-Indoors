...
class MotionService {
  ...
  private detectActivityFromBuffer(): void {
    ...
    // Enhanced classification using variance and step frequency from JSON spec
    ...
    const requiredVotes = this.lastActivity === 'SIT' ? 4 : 3;
    ...
    if (detectedActivity !== this.lastActivity &&
      ...
      if (this.lastActivity === 'SIT' && confidence < 0.90) {
