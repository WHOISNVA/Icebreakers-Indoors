...
import ARVIOFusionService, { VIOPose } from './ARVIOFusionService';
import BeaconUWBService, { IndoorPosition } from './BeaconUWBService';
...
class FusedPositioningService {
  ...
  async startTracking(): Promise<void> {
    ...
    try {
      ...
      await this.spatial3DService!.initialize();
  ...
  private handleVIOUpdate(pose: VIOPose): void {
    console.log(`VIO update: confidence ${pose.confidence}`);
  }

  private handleBeaconPosition(position: IndoorPosition): void {
