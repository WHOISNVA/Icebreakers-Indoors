...
import ExpoGoMotionService, { ARPose, VIOState } from './ExpoGoMotionService';
import ExpoGoBeaconService, { TriangulationResult } from './ExpoGoBeaconService';
import ExpoGoMeshService, { MeshNode, DeliveryOptimization } from './ExpoGoMeshService';

// Types are now imported from the individual Expo Go services
...
class FusedPositioningService {
  ...
  constructor(config: FusedPositioningConfig = {}) {
    this.config = {
      ...
      enableVIO: config.enableVIO ?? true,
      enableBeacons: config.enableBeacons ?? true,
      enableMesh: config.enableMesh ?? true,  // Now always enabled with Expo Go version
  ...
  private async initializeServices(): Promise<void> {
    ...
    this.gnssService = new ExpoGoLocationService({
      onLocationUpdate: (location: FilteredLocation) => this.handleGNSSUpdate(location),
      onLocationError: (error: any) => console.error('GNSS Error:', error),
    ...
    // Initialize VIO service
    if (this.config.enableVIO) {
      this.vioService = new ExpoGoMotionService({
        onPoseUpdate: (pose: ARPose) => this.handleVIOUpdate(pose),
        imuUpdateRate: 60
      });
    }
    ...
    if (this.config.enableBeacons) {
      this.beaconService = new ExpoGoBeaconService({
        onPositionUpdate: (result: TriangulationResult) => this.handleBeaconUpdate(result),
        onBeaconDetected: (beacon: any) => console.log(`Beacon detected: ${beacon.id} at ${beacon.distance}m`),
    ...
    if (this.config.enableMesh) {
      this.meshService = new ExpoGoMeshService({
  ...
  getCurrentDeck(): number | null {
    ...
    for (const [deck, elevation] of Array.from(this.cruiseShipSettings.deckElevations)) {
