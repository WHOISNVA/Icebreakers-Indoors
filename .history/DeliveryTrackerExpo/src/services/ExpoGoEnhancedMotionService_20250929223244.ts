  ...
  async startEnhancedMonitoringWithAutoDetection(): Promise<void> {
    ...
    if (isShipDetected.isOnShip) {
      ...
      this.enhancedConfig.onCruiseShipDetected?.({
        ...
        shipGPSReference: {
          latitude: isShipDetected.currentLocation?.latitude || 0,
          longitude: isShipDetected.currentLocation?.longitude || 0,
          altitude: isShipDetected.currentLocation?.altitude || 0,
          timestamp: Date.now()
        },
