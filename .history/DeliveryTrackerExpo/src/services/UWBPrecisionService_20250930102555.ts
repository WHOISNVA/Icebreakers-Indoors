...
class UWBPrecisionService {
  ...
  async initialize(): Promise<boolean> {
    try {
      ...
      if (this.compatibility.isExpoGo()) {
        console.error(
          '❌ UWB functionality requires native modules not available in Expo Go.\n' +
          'To use UWB features:\n' +
          '1. Run: npx expo prebuild\n' +
          '2. Install native UWB libraries\n' +
          '3. Build a custom development client: npx expo run:ios\n' +
          'Learn more: https://docs.expo.dev/development/build/'
        );
        return false;
      }

      // Check if device supports UWB
      if (!this.compatibility.getStatus().uwbSupported) {
        console.warn('UWB not supported on this device');
        return false;
      }

      // Real device detection for custom dev builds
  ...
  private startRanging(): void {
    ...
    const rangingLoop = setInterval(() => {
      ...
      Array.from(this.session.anchors.entries()).forEach(([anchorId, anchor]) => {
  ...
  private async performRanging(anchorId: string): Promise<void> {
    ...
    try {
      // Real UWB ranging - requires custom development build
      const anchor = this.session.anchors.get(anchorId);
      if (!anchor) return;
      ...
      // Update session quality to reflect failure
      if (this.session) {
        this.session.quality.accuracy *= 0.95; // Degrade accuracy on failure
      }
    }
  }

  private processMeasurement(anchorId: string, measurement: UWBMeasurement): void {
    // Store measurement
  ...
  private updateSessionQuality(measurement: UWBMeasurement): void {
    ...
    // quality.averageAccuracy = quality.averageAccuracy * (1 - alpha) + measurement.accuracy * alpha; // This line was removed from the new_code, so it's removed here.
  }
  ...
  private getRecentMeasurements(): UWBMeasurement[] {
    ...
    Array.from(this.measurements.values()).forEach((anchorMeasurements) => {
      const recent = anchorMeasurements
        .filter((m: UWBMeasurement) => m.timestamp > cutoffTime)
        .sort((a: UWBMeasurement, b: UWBMeasurement) => b.timestamp - a.timestamp)[0]; // Most recent
  ...
  getPerformanceMetrics(): {
    ...
    Array.from(this.measurements.entries()).forEach(([anchorId, measurements]) => {
      const recentMeasurements = measurements.filter((m: UWBMeasurement) => m.timestamp > Date.now() - 10000);
      const avgConfidence = recentMeasurements.length > 0
        ? recentMeasurements.reduce((sum: number, m: UWBMeasurement) => sum + m.quality.confidence, 0) / recentMeasurements.length
        : 0;
