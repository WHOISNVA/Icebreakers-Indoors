...
import ExpoGoLocationService from './ExpoGoLocationService';
import ExpoGoMotionService from './ExpoGoMotionService';
import ExpoGoBeaconService from './ExpoGoBeaconService';

// Conditional imports to avoid TurboModule loading in Expo Go
let AdvancedLocationService: any;
let ARVIOFusionService: any; 
let BeaconUWBService: any;
let MeshNetworkService: any;

// Import types separately to avoid runtime loading
import type { FilteredLocation, GNSSReading } from './AdvancedLocationService';
import type { ARPose, VIOState } from './ARVIOFusionService';
import type { TriangulationResult } from './BeaconUWBService';
import type { MeshNode, DeliveryOptimization } from './MeshNetworkService';
...
class FusedPositioningService {
  ...
  // Component services
  private gnssService!: any; // AdvancedLocationService or ExpoGoLocationService
  private spatial3DService!: Spatial3DService;
  private vioService?: any; // ARVIOFusionService or ExpoGoMotionService
  private beaconService?: any; // BeaconUWBService or ExpoGoBeaconService
  private meshService?: any; // MeshNetworkService or null in Expo Go
  ...
  constructor(config: FusedPositioningConfig = {}) {
    ...
    this.config = {
      ...
      enableVIO: isExpoGo ? false : config.enableVIO ?? true,
      enableBeacons: isExpoGo ? false : config.enableBeacons ?? true,
      enableMesh: isExpoGo ? false : config.enableMesh ?? true,
  ...
  private async initializeServices(): Promise<void> {
    // Dynamically import services to avoid TurboModule loading in Expo Go
