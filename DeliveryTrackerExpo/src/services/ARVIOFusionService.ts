...
import { Point3D, GeoPoint3D, PositionSource } from './Spatial3DService';
...
class ARVIOFusionService {
  ...
  async initialize(): Promise<boolean> {
    try {
      // Check compatibility
      if (!this.compatibility.getStatus().arSupported) {
        this.compatibility.logCompatibilityWarning('AR/VIO features');
        console.log('AR/VIO will use sensor-based positioning in Expo Go');
      }
      ...
      console.log('ARVIOFusion Service initialized');
      return true;
    } catch (error) {
      console.error('Failed to initialize ARVIOFusion service:', error);
      return false;
    }
  }
  ...
  private async initializeSensors(): Promise<void> {
    try {
      ...
      const [accelAvailable, gyroAvailable, magnetoAvailable] = await Promise.all([
      ...
      if (!accelAvailable || !gyroAvailable) {
        throw new Error('Required motion sensors not available');
      }
      ...
      if (magnetoAvailable) {
        Magnetometer.setUpdateInterval(100); // 10Hz for compass
      }
  ...
  // Marker detection removed - phones act as beacons only
  // VIO relies on IMU integration and GNSS fusion for positioning
