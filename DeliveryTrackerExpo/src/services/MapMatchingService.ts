...
class MapMatchingService {
   ...
   */
  public matchLocation(location: LocationData | Location.LocationObject): MatchedLocation {
    const lat = 'coords' in location ? location.coords.latitude : location.latitude;
    const lon = 'coords' in location ? location.coords.longitude : location.longitude;
    const acc = 'coords' in location ? (location.coords.accuracy ?? 0) : location.accuracy;
    const ts = 'coords' in location ? location.timestamp : location.timestamp;
    ...
    for (const zone of this.zones) {
      ...
      if (distance <= zone.radius) {
        ...
        const confidence = this.calculateConfidence(distance, zone.radius, acc);
   ...
   */
  public findNearestZone(location: LocationData | Location.LocationObject): { zone: Zone; distance: number } | null {
    const lat = 'coords' in location ? location.coords.latitude : location.latitude;
    const lon = 'coords' in location ? location.coords.longitude : location.longitude;
   ...
   */
  public snapToZone(location: LocationData | Location.LocationObject): LocationData {
    const lat = 'coords' in location ? location.coords.latitude : location.latitude;
    const lon = 'coords' in location ? location.coords.longitude : location.longitude;
    const acc = 'coords' in location ? (location.coords.accuracy ?? 0) : location.accuracy;
    const ts = 'coords' in location ? location.timestamp : location.timestamp;
    ...
    if (matched.matchedZone) {
      return {
        ...
        accuracy: Math.min(acc, 5), // Improve accuracy when snapped
        timestamp: ts,
        speed: matched.speed,
        heading: matched.heading,
        altitude: matched.altitude,
        altitudeAccuracy: matched.altitudeAccuracy,
      };
    }
